...
buildList start count end
  | count == 0 =
      [end]
  | otherwise =
      start : buildList start (count - 1) end
...
sorted :: [Int] -> Bool
sorted xs = sorted' Nothing xs
 where
  sorted' Nothing [] = True
  sorted' Nothing (y : ys) = sorted' (Just y) ys
  sorted' (Just x) [] = True
  sorted' (Just x) (y : ys) = if x > y then False else sorted' (Just y) ys
...
sumsOf xs = sumsOf' 0 xs
 where
  sumsOf' _ [] = []
  sumsOf' s (x : xs) = (s + x) : sumsOf' (s + x) xs
