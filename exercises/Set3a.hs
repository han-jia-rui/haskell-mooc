...
firstHalf s
  | even len = take halfLen s
  | otherwise = take (halfLen + 1) s
 where
  len = length s
  halfLen = len `div` 2
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k ^ x | x <- [0, 1 ..]]
...
sumRights = sum . map (\(Right x) -> x) . filter isRight
...
interpreter [] = []
interpreter commands = runcommand 0 0 commands

runcommand :: Integer -> Integer -> [String] -> [String]
runcommand x y [] = []
runcommand x y (c : cs) = case c of
  "up" -> runcommand x (y + 1) cs
  "down" -> runcommand x (y - 1) cs
  "left" -> runcommand (x - 1) y cs
  "right" -> runcommand (x + 1) y cs
  "printX" -> show x : runcommand x y cs
  "printY" -> show y : runcommand x y cs
