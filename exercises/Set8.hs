...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = Color (average r1 r2) (average g1 g2) (average b1 b2)
 where
  average x y = div (x + y) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture p1) (Picture p2) = Picture p3
 where
  p3 coord = f (p1 coord) (p2 coord)
...
union (Shape f1) (Shape f2) = Shape g
 where
  g coord = f1 coord || f2 coord

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape g
 where
...
paintSolid color (Shape isInShape) (Picture origin) = Picture new
 where
  new coord
    | isInShape coord =
        color
...
paint (Picture pat) (Shape isInShape) (Picture base) = Picture new
 where
  new coord
    | isInShape coord =
        pat coord
...
instance Transform Zoom where
  apply (Zoom x) pic = zoom x pic

data Flip = FlipX | FlipY | FlipXY
  deriving (Show)

instance Transform Flip where
  apply FlipX (Picture base) = Picture (base . (\(Coord x y) -> Coord (-x) y))
  apply FlipY (Picture base) = Picture (base . (\(Coord x y) -> Coord x (-y)))
...
averageColor colors = Color red green blue
 where
  red = (sum . map getRed $ colors) `div` length colors
  green = (sum . map getGreen $ colors) `div` length colors
  blue = (sum . map getBlue $ colors) `div` length colors

instance Transform Blur where
  apply Blur (Picture base) = Picture new
   where
    new coord = averageColor colors
     where
      xUp = base . (\(Coord x y) -> Coord (x + 1) y)
      xDown = base . (\(Coord x y) -> Coord (x - 1) y)
      yUp = base . (\(Coord x y) -> Coord x (y + 1))
      yDown = base . (\(Coord x y) -> Coord x (y - 1))
      pics = [base, xUp, xDown, yUp, yDown]
      colors = map ($ coord) pics
...
instance Transform BlurMany where
  apply (BlurMany n) pic = iterate (apply Blur) pic !! n
