...
longest :: (Ord a) => [[a]] -> [a]
longest xss =
  minimum . filter (\xs -> length xs == maxlen) $ xss
 where
...
freqs xs = foldr (\x m -> Map.alter update x m) Map.empty xs
 where
...
transfer from to amount bank
  | amount < 0 =
      bank
  | Map.notMember from bank || Map.notMember to bank =
      bank
  | famount < amount =
      bank
  | otherwise =
      Map.insert from (famount - amount) . Map.insert to (tamount + amount) $ bank
 where
  famount = bank Map.! from
  tamount = bank Map.! to
...
maxIndex arr = fst . maximumBy (\(i1, e1) (i2, e2) -> compare e1 e2) $ assocs arr
