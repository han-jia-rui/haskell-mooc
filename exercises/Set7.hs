...
data State = Start | Error | Finished | FirstState | Second1 | Second2 | Third | Forth
  deriving (Eq, Show)

step :: State -> Event -> State
step st ev = case (st, ev) of
  (Start, AddEggs) -> FirstState
  (FirstState, AddFlour) -> Second1
  (FirstState, AddSugar) -> Second2
  (Second1, AddSugar) -> Third
  (Second2, AddFlour) -> Third
  (Third, Mix) -> Forth
  (Forth, Bake) -> Finished
  (Finished, _) -> Finished
  (_, _) -> Error
...
average (x :| xs) =
  let (total, len) = foldl' (\(s, l) v -> (s + v, l + 1)) (x, 1) xs
...
instance (Ord a) => Semigroup (Set a) where
  Set xs <> Set ys = foldr (\x acc -> add x acc) (Set ys) xs
...
passwordAllowed pw (ContainsSome xs) =
  foldr (\x acc -> if elem x pw then True else acc) False xs
passwordAllowed pw (DoesNotContain xs) =
  foldr (\x acc -> if elem x pw then False else acc) True xs
passwordAllowed pw (And req1 req2) = and . map (passwordAllowed pw) $ [req1, req2]
passwordAllowed pw (Or req1 req2) = or . map (passwordAllowed pw) $ [req1, req2]
...
evaluate (Operation s a1 a2) = case s of
...
render (Operation s a1 a2) = case s of
  "+" -> "(" ++ render a1 ++ "+" ++ render a2 ++ ")"
  "-" -> "(" ++ render a1 ++ "-" ++ render a2 ++ ")"
  "*" -> "(" ++ render a1 ++ "*" ++ render a2 ++ ")"
