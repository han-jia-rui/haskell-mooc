...
setName name (MkPerson _ age) = MkPerson name age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson name _) = MkPerson name age
...
rgb col = case col of
  ...
  Green -> [0, 1, 0]
  Blue -> [0, 0, 1]
  Mix col1 col2 -> zipWith (\x y -> (x + y) / 2) (rgb col1) (rgb col2)
  Invert col1 -> map (\x -> 1 - x) (rgb col1)
...
toList kvs = case kvs of
  Empty -> []
  Pair k v kv -> (k, v) : toList kv
...
fromNat :: Nat -> Int
fromNat nat = case nat of
  Zero -> 0
  PlusOne nat' -> 1 + fromNat nat'
